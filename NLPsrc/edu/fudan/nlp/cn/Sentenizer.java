...
public class Sentenizer {
	...
	public static String[] split(String strSent) {
		List<Integer> plist = new ArrayList<Integer>();
		int p = 0;
		for (int i = 0; i < puncs.length; i++) {
			p = strSent.indexOf(puncs[i]);
			while (p != -1) {
				plist.add(p);
				p = strSent.indexOf(puncs[i], p + 1);
			}
		}
		Collections.sort(plist);
		if (!plist.isEmpty()) {
			p = plist.get(plist.size() - 1);
			if (p < strSent.length() - 1)
				plist.add(strSent.length() - 1);
		}else	{
			plist.add(strSent.length() - 1);
		}
		...
		for (int i = 0; i < plist.size(); i++) {
			ret[i] = strSent.substring(p, plist.get(i) + 1);
			p = plist.get(i) + 1;
		}
		plist.clear();
